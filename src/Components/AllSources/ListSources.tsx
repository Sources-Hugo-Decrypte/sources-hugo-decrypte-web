                                ...
                                <tbody className="sm:contents">
                                    {
                                        sourcesData.map(source => (
                                            <tr className="grid grid-cols-5 mb-4 sm:text-justify sm:contents group" key={source.name + '-row'}>
                                                ...
                                                <td className="col-span-4 pt-3 pl-2 overflow-hidden whitespace-nowrap text-ellipsis font-bold text-lg sm:text-sm sm:col-span-1 sm:p-0 sm:py-1 sm:font-normal sm:group-odd:bg-gray-100">{source.name}</td>
                                                <td className="col-span-full pl-1.5 py-2 text-sm overflow-hidden whitespace-nowrap text-ellipsis sm:col-span-1 sm:p-0 sm:py-1 sm:group-odd:bg-gray-100"><LinkIcon className="w-4 h-4 sm:hidden inline-block" /> {source.totalLinks} <span className="sm:hidden">liens soit {source.percentage} %</span></td>
                                                <td className="hidden text-sm overflow-hidden whitespace-nowrap text-ellipsis col-span-1 py-1 sm:block sm:group-odd:bg-gray-100">{source.percentage} %</td>
                                                <td className="col-span-full pl-1.5 py-2 text-sm rounded-r overflow-hidden whitespace-nowrap text-ellipsis sm:col-span-1 sm:p-0 sm:py-1 sm:group-odd:bg-gray-100"> <CalendarIcon className="w-4 h-4 sm:hidden inline-block" /> {new Date(source.lastDate).toLocaleDateString('fr-FR')}</td>
                                            </tr>
                                        ))
                                    }
                                </tbody>
